...
std::vector <std::wstring> editors;
std::string procName;
    ...
    m_dataAccess(dataAccess), m_nextPictureId(100), m_nextUserId(200)
{
	// Left empty
	m_dataAccess.open();
	m_dataAccess.getHighstUserId() == 0 ? m_nextUserId : m_nextUserId = m_dataAccess.getHighstUserId() + 1;
	m_dataAccess.getHighstPictureId() == 0 ? m_nextPictureId : m_nextPictureId = m_dataAccess.getHighstPictureId() + 1;
}
...
void AlbumManager::showPicture()
{
	...
	std::wstring path(pic.getPath().begin(), pic.getPath().end());
	std::cout << "open in : \n1 - paint\n2- something else\n3 - copy file to anthor place";
	int choise;
	std::cin >> choise;
	if (choise == 1) {
		ShellExecuteW(NULL, L"open", L"C:\\Windows\\system32\\mspaint.exe", (LPCWSTR)path.c_str(), NULL, SW_SHOW);
	}
	if (choise == 2) 
	{
		std::cout << "choose photo editor : (-1 to enter new one)\n";
		int count = 0;
		for (auto i : editors) 
		{
			std::wcout << count << ". " << i << "\n";
		...
		int c;
		std::wstring pathToEditor;
		std::cin >> c;
		if (c != -1 || (c > 0 || c < editors.size())) {
			std::cout << "inside";
			ShellExecuteW(NULL, L"open", (LPCWSTR)editors[c].c_str(), (LPCWSTR)path.c_str(), NULL, SW_SHOW);
			goto end;
		}
		std::cout << "enter path to other photo editor : ";
		std::wcin >> pathToEditor;
		editors.push_back(pathToEditor);
		ShellExecuteW(NULL, L"open",(LPCWSTR)pathToEditor.c_str() , (LPCWSTR)path.c_str(), NULL, SW_SHOW);
		end: {};
	}
	if (choise == 3) 
	{
		std::string newPath;
		std::cout << "enter path to copy file to : ";
		std::cin >> newPath;
		CopyFile(LPCSTR(pic.getPath().c_str()), LPCSTR(newPath.c_str()), true);
	}
}
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		...
		"  + Albums user own :" << m_dataAccess.getAlbumsOfUser(user).size() << std::endl;
}
